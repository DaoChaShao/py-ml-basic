...
# -*- Coding: UTF-8 -*-
# @Time     :   2026/9/23 01:23
# @Author   :   Shawn
# @Version  :   Version 0.1.0
# @File     :   04_diabetes_reg_04_lasso_diagnosis.py
# @Desc     :
...
def main() -> None:
    ...
    with FeaturesRobustScaler(train_features) as standardiser:
        ...
        train_predictions = lasso.predict(poly_train_features)
        train_metrics = lasso.eval_reg(train_labels, train_predictions, display=False)
        train_rmse: float = train_metrics["rmse"]

        valid_predictions = lasso.predict(poly_valid_features)
        valid_metrics = lasso.eval_reg(valid_labels, valid_predictions, display=True)
