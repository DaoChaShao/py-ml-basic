...
from utils.ml.estimators import NaiveBayes
...
def main() -> None:
    ...
    with FeaturesNormaliser(train_features) as standardiser:
        ...
        bayes = NaiveBayes(0.1, force_alpha=True)
        ...
        bayes = search.best_estimator_

        train_predictions = search.predict(train_features)
        train_metrics = bayes.eval_cls(train_labels, train_predictions, display=False)
        train_f1: float = train_metrics["f1_score"]

        valid_predictions = search.predict(valid_features)
        valid_metrics = bayes.eval_cls(valid_labels, valid_predictions, display=True)
        ...
        ****************************************************************
        ****************************************************************
        Classification Evaluation Metrics - Native Bayes
        ----------------------------------------------------------------
        Accuracy  : 0.7727
        Precision : 0.7882
        Recall    : 0.7727
        ...
        ****************************************************************
        ...
        status, pred_label = bayes.inference(prove_features.iloc[[row]], prove_labels.iloc[row],
                                             display=False)
        if status:
            print(
                f"{green('Correct')}! "
        ...
        pred = bayes.predict(sample_features)[0]
        print(f"Prediction: {list(IrisLabels)[pred].CN}")
        probs = bayes.confidence(sample_features)[0]
